<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('tbl_distributor_porder_detail', function (Blueprint $table) {
            $table->increments('idtbl_porder_detail');
            $table->double('qty')->default(0);
            $table->double('unitprice')->default(0);
            $table->double('saleprice')->default(0);
            $table->double('retailprice')->nullable();
            $table->double('total')->default(0);
            $table->integer('product_id');
            $table->integer('porder_id');
            $table->integer('status')->nullable();
            $table->dateTime('insertdatetime');
            $table->integer('updateuser')->nullable();
            $table->dateTime('updatedatetime')->nullable();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('tbl_distributor_porder_detail');
    }
};
